...
class ServiceSettings(Service):
    class API(Group):
        SERVICE_NAME = "sta"
        SERVICE_DESCRIPTION = "REST API for Sensorthings API"
        SERVICE_VERSION = "v1.1.2"
        DEFAULT_DATE_TIME_FORMAT = "%y-%m-%d %H:%M:%S:%f"
        SERVICE_ACCESS_ROLE_NAME = "access"
        SERVICE_ADMIN_ROLE_NAME = "admin"
        SERVICE_HOST = "sta-sta.apps.paas-dev.psnc.pl"
        # SERVICE_HOST = "localhost.sta"
        SERVICE_PROTOCOL_PREFIX = "https://"
        SERVICE_API_PREFIX = "api"

    class Main(Group):
        DEBUG = False
        SECRET_KEY = "vl@^dt&+!2gbol*os@5=^=xx4(wz6c9*b-ch7+nffh7ab%*403"
        MAIN_DOMAIN = "localhost.sta"
    ...
    class Auth(Group):
        ...
        AUTH_FAKE_OPENID_PROVIDER = "https://fake.login.sta.org"
...
if SENTRY_DSN:  # pragma: no cover # noqa
    ...
    sentry_sdk.init(
        dsn=SENTRY_DSN, environment=SENTRY_ENVIRONMENT, integrations=[DjangoIntegration()]  # noqa
    )
